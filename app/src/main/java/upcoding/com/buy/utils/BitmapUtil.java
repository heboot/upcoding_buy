...
public class BitmapUtil {
    ...
    public static String saveBitmapFile(Bitmap bitmap) {
        String sdPath = FileUtils.initPath();
        String path = sdPath + FileUtils.SD_IMG_PATH;
        File file = new File(path);//将要保存图片的路径
        if (!file.exists()) {
            file.mkdir();
        }
        File imgFile = new File(path + System.currentTimeMillis() + ".jpg");
        try {
    ...
    public static String saveBitmapFile(Bitmap bitmap, boolean isHead) {
        String sdPath = FileUtils.initPath();
        String path = sdPath + FileUtils.SD_IMG_PATH;
        File file = new File(path);//将要保存图片的路径
        if (!file.exists()) {
        ...
        File imgFile = new File(path + "avatar.jpg");
        try {
            BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(imgFile));
            bitmap.compress(Bitmap.CompressFormat.JPEG, 100, bos);
            bos.flush();
...
//        image.compress(Bitmap.CompressFormat.JPEG, 100, baos);
        ...
        if (length / 1024 > 1024) {//判断如果图片大于1M,进行压缩避免在生成图片（BitmapFactory.decodeStream）时溢出
            baos.reset();//重置baos即清空baos
